...
(0, _helpers.extend)(Serenade, Object.defineProperties({
	...
	helper: function helper(name, fn) {
		_context2["default"][name] = function () {
			...
			return _viewsDynamic_view2["default"].bind(_channel2["default"].all(args), function (view, args) {
				var result = fn.apply({
					context: _this,
				...
				if (!result) {
					...
					view.replace(new _viewsView2["default"](result));
				}
			});
		};
	},
	...
	template: function template(nameOrTemplate, _template) {
		if (_template) {
			return this.templates[nameOrTemplate] = new _template3["default"](_template);
		} else {
			return new _template3["default"](nameOrTemplate);
		}
	},
