...
var _defineEvent = require("../event");

var _defineEvent2 = _interopRequireDefault(_defineEvent);
...
var Property = {
  style: {
    update: function update(property, value) {
      _settings$assignUnlessEqual.assignUnlessEqual(this.node.style, property.name, value);
    }
  },
  event: {
  ...
  binding: {
    setup: function setup(property) {
      ...
      if (property.name === "binding") {
        (function () {
          ...
          _settings$assignUnlessEqual.settings.document.addEventListener("submit", handler, true);
          _this2.unload.bind(function () {
            _settings$assignUnlessEqual.settings.document.removeEventListener("submit", handler, true);
          });
        })();
      } else {
    ...
    update: function update(property, value) {
      if (this.node.type === "checkbox") {
        ...
        return _settings$assignUnlessEqual.assignUnlessEqual(this.node, "value", value);
      }
    }
  },
  attribute: {
    update: function update(property, value) {
      if (property.name === "value") {
        _settings$assignUnlessEqual.assignUnlessEqual(this.node, "value", value || "");
      } else if (this.ast.name === "input" && property.name === "checked") {
        _settings$assignUnlessEqual.assignUnlessEqual(this.node, "checked", !!value);
  ...
  property: {
    update: function update(property, value) {
      _settings$assignUnlessEqual.assignUnlessEqual(this.node, property.name, value);
    }
  }
};
...
var Element = (function (_View) {
  function Element(ast, context) {
    ...
    _get(Object.getPrototypeOf(Element.prototype), "constructor", this).call(this, _settings$assignUnlessEqual.settings.document.createElement(ast.name));
  ...
  _createClass(Element, [{
    ...
    value: function _updateClass() {
      ...
      if (classes.length) {
        _settings$assignUnlessEqual.assignUnlessEqual(this.node, "className", classes.sort().join(" "));
      } else {
...
_defineEvent2["default"](Element.prototype, "load", { async: false });
_defineEvent2["default"](Element.prototype, "unload", { async: false });

_Compile2["default"].element = function (ast, context) {
  if (_settings$assignUnlessEqual.settings.views[ast.name]) {
    return _settings$assignUnlessEqual.settings.views[ast.name](ast, context);
  } else {
